...
async function handleRsvp(req, supabase, userId) {
    ...
    // Fallback: if the RPC doesn't exist yet, do it in application code
    ...
    if (error?.message?.includes("function") && error?.message?.includes("does not exist")) {
        ...
        const { data: rsvp, error: rsvpError } = await supabase
            .from("rsvps")
            .upsert({ event_id, user_id: userId, status, updated_at: new Date().toISOString() }, { onConflict: "event_id,user_id" })
            .select()
            .single();
        if (rsvpError) {
...
async function handleCreateEvent(req, supabase, userId) {
    const body = await req.json();
    const { type, title, format, city, starts_at, venue_id, duration_min, min_players, max_players, fee_text, description, cloned_from } = body;
...
// LFG signal handler
async function handleLfg(req, supabase, userId) {
    const { city, formats } = await req.json();
...
async function handleAssignRole(req, supabase, adminId) {
